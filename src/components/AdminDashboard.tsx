...
import { LogOut, Users, Calendar, Shield, CheckSquare, BarChart2, MessageSquare } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'events' | 'guilds' | 'codes' | 'stats' | 'chat'>('users');
  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex flex-col md:flex-row pb-16 md:pb-0 font-sans">
      ...
      <div className="hidden md:flex w-64 bg-white border-r border-zinc-200 flex-col fixed h-full z-10">
        <div className="p-6">
          ...
          <p className="text-xs font-medium text-zinc-500 uppercase tracking-wider mt-3 ml-1">Админ</p>
        </div>
        
        <div className="px-6 pb-4">
          <div className="bg-zinc-100 rounded-xl p-4">
            <p className="text-sm font-medium text-zinc-900 truncate">{userProfile?.name}</p>
            <p className="text-xs text-zinc-500 mt-1">Администратор</p>
          </div>
        </div>
      ...
      <div className="md:hidden bg-white/80 backdrop-blur-md border-b border-zinc-200 p-4 sticky top-0 z-20 flex justify-between items-center">
        <div>
          ...
          <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-wider ml-0.5">Админ</p>
        </div>
      ...
      {/* Main Content */}
      <div className="flex-1 md:ml-64 p-4 md:p-8 overflow-y-auto w-full max-w-7xl mx-auto">
