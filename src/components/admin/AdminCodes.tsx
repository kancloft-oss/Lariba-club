...
export default function AdminCodes() {
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-0">
      ...
      {/* Desktop Table View */}
      <div className="hidden md:block bg-white shadow-sm border border-zinc-100 rounded-2xl overflow-hidden">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                      ${code.completedCount >= code.totalRequired ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-amber-50 text-amber-700 border border-amber-100'}`}>
      ...
      <div className="md:hidden space-y-4">
        {filteredCodes.map((code) => (
          <div key={code.id} className="bg-white p-5 rounded-2xl shadow-sm border border-zinc-100 flex flex-col gap-4">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-base font-bold text-zinc-900">{code.title}</h3>
                <div className="flex flex-col mt-1">
                  <p className="text-sm font-medium text-zinc-500">{getUserName(code.residentId)}</p>
                  <div className="mt-1">
                    {users.find(u => u.uid === code.residentId)?.tariff && (
                      <TariffBadge tariff={users.find(u => u.uid === code.residentId)!.tariff} />
                    )}
                  </div>
                  <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider flex items-center mt-1">
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <select required value={residentId} onChange={(e) => setResidentId(e.target.value)} className="block w-full border border-zinc-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-zinc-900 focus:border-zinc-900 sm:text-sm transition-all bg-white text-zinc-900">
                  ...
                  <div>
                    ...
                    <input type="text" required value={title} onChange={(e) => setTitle(e.target.value)} className="block w-full border border-zinc-200 bg-white text-zinc-900 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-zinc-900 focus:border-zinc-900 sm:text-sm transition-all" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-zinc-700 mb-1.5">Описание</label>
                    <textarea required value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="block w-full border border-zinc-200 bg-white text-zinc-900 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-zinc-900 focus:border-zinc-900 sm:text-sm transition-all resize-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-zinc-700 mb-1.5">Дедлайн</label>
                    <input type="date" required value={deadline} onChange={(e) => setDeadline(e.target.value)} className="block w-full border border-zinc-200 bg-white text-zinc-900 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-zinc-900 focus:border-zinc-900 sm:text-sm transition-all" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-zinc-700 mb-1.5">Тип разбивки</label>
                    <select required value={frequencyType} onChange={(e) => setFrequencyType(e.target.value as any)} className="block w-full border border-zinc-200 bg-white text-zinc-900 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-zinc-900 focus:border-zinc-900 sm:text-sm transition-all">
                      ...
                      <div className="flex gap-2">
                        {['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'].map((day, i) => (
                          <button
                            ...
                            className={`px-3 py-2 rounded-lg text-xs font-bold ${schedule.includes((i + 1).toString()) ? 'bg-zinc-900 text-white' : 'bg-zinc-100 text-zinc-600'}`}
                          >
                            {day}
                          </button>
                        ))}
