              ...
              <div className="max-h-60 overflow-y-auto">
                {availableMonths.map(month => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm font-medium hover:bg-zinc-50 transition-colors ${selectedMonth === month ? 'bg-zinc-900 text-white hover:bg-zinc-800' : 'text-zinc-700'}`}
                  >
                    {formatMonth(month)}
                  </button>
                ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white shadow-sm border border-zinc-200 rounded-2xl overflow-hidden">
              <div className="px-6 py-6 border-b border-zinc-100 bg-zinc-900 text-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <ul className="divide-y divide-zinc-100">
                {guildMembers.map((member, index) => {
                  ...
                  return (
                    <li key={member.uid} className={`px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all ${isMe ? 'bg-zinc-50 border-l-4 border-zinc-900' : 'hover:bg-zinc-50/50 border-l-4 border-transparent'}`}>
                      <div className="flex items-center">
                        ...
                        <div>
                          <p className="text-sm font-bold text-zinc-900 flex items-center">
                            {member.name}
                            {isMe && <span className="ml-2 px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider bg-zinc-900 text-white">Вы</span>}
                          </p>
            ...
            <div className="bg-white shadow-sm border border-zinc-200 rounded-2xl overflow-hidden sticky top-24">
              ...
              <ul className="divide-y divide-zinc-100">
                {sortedGuildsByMonthly.map((guild, index) => {
                  ...
                  return (
                    <li key={guild.id} className={`px-6 py-4 flex items-center justify-between transition-colors ${isMyGuild ? 'bg-zinc-900 text-white' : 'hover:bg-zinc-50/50'}`}>
                      <div className="flex items-center">
                        {index === 0 ? <Medal className={`mr-3 ${isMyGuild ? 'text-amber-400' : 'text-amber-500'}`} size={20} /> :
                         ...
                         <span className={`w-5 text-center font-bold mr-3 ${isMyGuild ? 'text-zinc-400' : 'text-zinc-400'}`}>{index + 1}</span>}
